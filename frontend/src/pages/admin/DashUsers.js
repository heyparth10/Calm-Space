  ...
  const deleteUserById = (e,id) => {
    ...
    const columns = [
...

        {
            ...
            width: 150,
            renderCell: (params) => (
                params.row.role === 1 ? "Admin" : (params.row.role === 0 ? "Regular user" : "Blog Creator")
            )
        },
...

        {
            ...
            renderCell: (values) => (
                <Box sx={{ display: "flex", justifyContent: "space-between", width: "170px" }}>
                    <Button variant="contained"><Link style={{ color: "white", textDecoration: "none" }} to={`/admin/user/edit/${values.row._id}`}>Edit</Link></ Button>
    ...
    return (
        <>
            <Box className="pl-6 pt-6 pr-6">

                <Typography variant="h4" sx={{ color: "white", pb: 3 }}>
                    All users
                </Typography>
                <Box sx={{ pb: 2, display: "flex", justifyContent: "right" }}>
                    <Button variant='contained' color="success" startIcon={<AddIcon />}><Link style={{ color: "white", textDecoration: "none" }} to="/register">Create User</Link></Button>
                </Box>
