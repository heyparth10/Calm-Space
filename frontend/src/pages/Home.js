...
const [predictedAnswer, setPredictedAnswer] = React.useState("");
  ...
  const predictDisorder = (answersArray) => {
    ...
    const answersIntegers = answersArray.map(answer => parseInt(answer));
    console.log(answersIntegers)


    const anxietyScore = answersIntegers.slice(0, 5).reduce((acc, curr) => acc + curr, 0);
    const depressionScore = answersIntegers.slice(5, 10).reduce((acc, curr) => acc + curr, 0);

    console.log(anxietyScore, depressionScore)
    if (anxietyScore > depressionScore) {
      ...
      return 'Further assessment needed. You can consult a mental health professional';
    }
  };
  ...
  const topics = [
    { name: 'Anxiety', link: 'blog/anxiety' },
    { name: 'Depression', link: 'blog/depression' },
