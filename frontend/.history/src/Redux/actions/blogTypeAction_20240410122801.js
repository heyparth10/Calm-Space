...
export const createBlogTypeAction = (blogtype) => async (dispatch) => {
    ...
    try {
        const { data } = await axios.post(`${backend_api}api/type/create`, blogtype)
        dispatch({
...
export const editBlogTypeAction = (blogTypeID, newBlogTypeName) => async (dispatch) => {
    ...
    try {
        const { data } = await axios.put(`${backend_api}api/type/update/${blogTypeID}`,
        {
...
export const blogTypeDeleteAction = (blogTypeID) => async (dispatch) => {
    try {
        const { data } = await axios.delete(`${backend_api}/api/type/delete/${blogTypeID}`);
